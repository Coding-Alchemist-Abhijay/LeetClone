...
export async function POST(req) {
    try {
    const {source_code , language_id , stdin , expected_outputs , id} = await req.json();
    ...
    if (
        ...
        return NextResponse.json({ success: false, error: "Invalid test cases" }, {statusCode : 400});
        }
    ...
    return NextResponse.json({ success: true, submission: submissionWithTestCases }, {statusCode : 200});
    } catch (error) {
        console.error(error);
        return NextResponse.json(
