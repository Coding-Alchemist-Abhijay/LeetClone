...
import { SubmissionDetails } from "@/components/ui/submission-details";
import { TestCaseTable } from "@/components/ui/test-case-table";
...
const ProblemIdPage = () => {
  ...
  useEffect(() => {
    const fetchProblem = async () => {
      try {;
        const id = params.id;
        const res = await fetch(`/api/get-problem/${id}`);
    ...
    const fetchSubmissionHistory = async()=>{
      try {
        ...
        const submissionHistory = await res.json();
  ...
  const handleRun = async () => {
    try {
      ...
      const result = await fetch("/api/submit-problem", {
        method: "POST",
        headers: {
